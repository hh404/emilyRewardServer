...
app.post('/api/update-settings', (req, res) => {
    const { totalTime, addTime5Min, addTime10Min } = req.body;
    const query = 'UPDATE rewardSetting SET totalTime = ?, addTime5Min = ?, addTime10Min = ? WHERE id = 1';

    db.query(query, [totalTime, addTime5Min, addTime10Min], (err, result) => {
        if (err) {
            console.error('Error executing query: ', err);
            res.status(500).send('Error executing query');
            return;
